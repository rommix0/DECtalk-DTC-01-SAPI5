// com.hpp - generic COM plumbing (IUnknown/IClassFactory implementation
// templates, a CLSID-keyed class_object_factory, and a class_registrar for
// writing/removing a coclass's InProcServer32 registration). Adapted from
// BstSpeech-sapi-master's src/com.hpp (namespace Bestspeech -> dectalk).
//
// This header carries no DECtalk-engine logic; it is brought in ahead of the
// task that implements DectalkTtsEngine (B2) and the task that wires up
// DllGetClassObject/DllRegisterServer (B1), both of which include it.
#pragma once
